...
use n2s::eval::{
    evaluate,
    score::{compute_score, ScoreWeights},
};
...
fn empty_input_errors_cleanly() {
    ...
    match convert_full("* just a comment\n", &opts) {
        ...
        Err(e) => assert!(
            e.to_lowercase().contains("no devices"),
            "wrong error: {}",
            e
        ),
    }
...
fn rc_lowpass_creates_power_symbol_for_ground() {
    ...
    assert!(
        ...
        "expected at least one power symbol for GND"
    );
}
...
fn diff_pair_pipeline_pairs_devices_symmetrically() {
    ...
    let q1 = s
        .components
        .iter()
        .find(|c| c.instance_name == "Q1")
        .expect("Q1 missing");
    let q2 = s
        .components
        .iter()
        .find(|c| c.instance_name == "Q2")
        .expect("Q2 missing");
    assert!(
    ...
    for name in &examples {
        ...
        if breakdown.overlap_score < 0.999 {
            failures.push(format!(
                "{}: overlap < 1.0 ({:.3})",
                name, breakdown.overlap_score
            ));
        }
        if breakdown.symmetry_score < 0.999 {
            failures.push(format!(
                "{}: symmetry < 1.0 ({:.3})",
                name, breakdown.symmetry_score
            ));
        }
        if breakdown.power_convention_score < 0.999 {
            failures.push(format!(
                "{}: power_convention < 1.0 ({:.3})",
                name, breakdown.power_convention_score
            ));
        }
    }
...
fn all_examples_run_through_pipeline_and_eval() {
    ...
    assert!(
        examples.len() >= 11,
        "expected ≥11 example netlists, found {}",
        examples.len()
    );
...
fn export_json_round_trip() {
    ...
    let parsed: n2s::model::Schematic = serde_json::from_str(&serialized).expect("deserialize");
...
fn obstacle_avoidance_keeps_wires_off_component_bodies() {
    ...
    let body_rects: Vec<(f64, f64, f64, f64)> = s_on
        .components
        .iter()
        .filter_map(|c| {
    ...
    fn count_body_intrusions(wires: &[n2s::model::Wire], rects: &[(f64, f64, f64, f64)]) -> usize {
        let mut count = 0;
        for wire in wires {
...
fn hierarchical_netlist_renders_top_level_by_default() {
    ...
    assert!(
        s.components.iter().any(|c| c.symbol_name == "subckt_INV"),
...
fn improve_search_runs_multiple_restarts_when_target_unreachable() {
    ...
    let final_score = report["final_score"].as_f64().unwrap();
    let max_per_restart = summaries
        .iter()
        .map(|s| s["best_score"].as_f64().unwrap())
        .fold(f64::NEG_INFINITY, f64::max);
    assert!(
        (final_score - max_per_restart).abs() < 1e-3,
...
fn export_kicad_writes_kicad_sch_envelope() {
    ...
    assert!(
        body.trim_start().starts_with("(kicad_sch"),
...
fn export_svg_writes_well_formed_xml() {
    ...
    assert!(
        body.contains("</svg>"),
        "SVG output should close the <svg> tag"
    );
    let _ = std::fs::remove_file(&tmp);
}
